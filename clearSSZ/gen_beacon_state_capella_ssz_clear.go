// Code generated by github.com/karalabe/ssz. DO NOT EDIT.

package gen_clear_ssz

// ClearSSZ zeroes out all fields of BeaconStateCapella for leftover decode.
func (obj *BeaconStateCapella) ClearSSZ() {
	obj.GenesisTime = 0
	obj.GenesisValidatorsRoot = [32]byte{}
	obj.Slot = 0
	if obj.Fork != nil {
		obj.Fork.ClearSSZ()
	}
	if obj.LatestBlockHeader != nil {
		obj.LatestBlockHeader.ClearSSZ()
	}
	obj.BlockRoots = [8192][32]byte{}
	obj.StateRoots = [8192][32]byte{}
	obj.HistoricalRoots = nil
	if obj.Eth1Data != nil {
		obj.Eth1Data.ClearSSZ()
	}
	for i := range obj.Eth1DataVotes {
		if obj.Eth1DataVotes[i] != nil {
			obj.Eth1DataVotes[i].ClearSSZ()
		}
	}
	obj.Eth1DepositIndex = 0
	for i := range obj.Validators {
		if obj.Validators[i] != nil {
			obj.Validators[i].ClearSSZ()
		}
	}
	obj.Balances = nil
	obj.RandaoMixes = [65536][32]byte{}
	obj.Slashings = [8192]uint64{}
	obj.PreviousEpochParticipation = nil
	obj.CurrentEpochParticipation = nil
	obj.JustificationBits = [1]byte{}
	if obj.PreviousJustifiedCheckpoint != nil {
		obj.PreviousJustifiedCheckpoint.ClearSSZ()
	}
	if obj.CurrentJustifiedCheckpoint != nil {
		obj.CurrentJustifiedCheckpoint.ClearSSZ()
	}
	if obj.FinalizedCheckpoint != nil {
		obj.FinalizedCheckpoint.ClearSSZ()
	}
	obj.InactivityScores = nil
	if obj.CurrentSyncCommittee != nil {
		obj.CurrentSyncCommittee.ClearSSZ()
	}
	if obj.NextSyncCommittee != nil {
		obj.NextSyncCommittee.ClearSSZ()
	}
	if obj.LatestExecutionPayloadHeader != nil {
		obj.LatestExecutionPayloadHeader.ClearSSZ()
	}
	obj.NextWithdrawalIndex = 0
	obj.NextWithdrawalValidatorIndex = 0
	for i := range obj.HistoricalSummaries {
		if obj.HistoricalSummaries[i] != nil {
			obj.HistoricalSummaries[i].ClearSSZ()
		}
	}
}
